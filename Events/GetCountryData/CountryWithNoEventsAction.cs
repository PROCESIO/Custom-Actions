...
public class CountryWithNoEventsAction : IAction
{
    ...
    [FEDecorator(Label = "Region Info", Type = FeComponentType.DataType, RowId = 6, Tab = "Geo")]
    [BEDecorator(IOProperty = Direction.Output)]
    public object? RegionInfo { get; set; }

    [FEDecorator(Label = "Country Info", Type = FeComponentType.DataType, RowId = 7, Tab = "Geo")]
    [BEDecorator(IOProperty = Direction.Output)]
    public object? CountryInfo { get; set; }

    [FEDecorator(Label = "Local Time", Type = FeComponentType.Text, RowId = 8, Tab = "Geo")]
    [BEDecorator(IOProperty = Direction.Output)]
    public string? CountryLocalTime { get; set; }

    [FEDecorator(Label = "Currency Info", Type = FeComponentType.DataType, RowId = 9, Tab = "Geo")]
    [BEDecorator(IOProperty = Direction.Output)]
    public object? CurrencyInfo { get; set; }

    [FEDecorator(Label = "Country Summary File", Type = FeComponentType.File, RowId = 10, Tab = "Geo")]
    [BEDecorator(IOProperty = Direction.Output)]
