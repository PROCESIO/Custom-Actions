...
namespace Actions
{
    ...
    [ClassDecorator(Name = "String StartsWith", Shape = ActionShape.Circle, Description = "Verify if a string starts with a " +
        "specified string", IsTestable = true, Classification = Classification.cat1)]
    [Permissions(CanDelete = true, CanDuplicate = true, CanAddFromToolbar = true)]
    public class StringStartsWith : IAction
    {
        #region Properties
        ...
        [FEDecorator(Label = "Input String - Start", Type = FeComponentType.Text, RowId = 2, Tab = "Configuration Tab")]
        [BEDecorator(IOProperty = Direction.Input)]
        [Validator(IsRequired = true, Expects = ExpectedType.String)]
        ...
        {
            if (InputString == null || InputStart == null)
            {
                throw new System.Exception("Input was null. Please, fill it");
            }

            Output = InputString.StartsWith(InputStart);
        }
        #endregion
    }
}
