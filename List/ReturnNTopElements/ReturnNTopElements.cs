...
{
    [ClassDecorator(Name = "Return Top N Elements", Shape = ActionShape.Square, Description = "Returns the top N elements", IsTestable = true, Classification = Classification.cat1)]
    [Permissions(CanDelete = true, CanDuplicate = true, CanAddFromToolbar = true)]
    public class ReturnNTopElements : IAction
    {
        ...
        public ReturnNTopElements(IServiceProvider service) { }
        ...
        public async Task Execute()
        {
            ...
            if (UserChoice == 1)
            {
                Result = InputList.Take(ElementsN);
            }
            else if (UserChoice == 3)
            {
                Result = OrderByHelper.GetSortedDesc(InputList.Take(ElementsN));
            }
            else if (UserChoice == 2)
            {
                Result = OrderByHelper.GetSortedAsc(InputList.Take(ElementsN));
            }
            else
            {
